...
// Main Page Component (Synchronous shell)
export default function ApartmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  return (
    <main className="min-h-screen bg-white pb-20">
      
      
      {/* Wrap the data-heavy part in Suspense */}
      <Suspense fallback={<ApartmentDetailSkeleton />}>
        <ApartmentDataFetcher params={params} />
      </Suspense>
    </main>
  );
}

// The Data Fetcher (Handles all async work)
async function ApartmentDataFetcher({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    ...
    <Breadcrumb />
      ...
      <section className="max-w-7xl mx-auto px-4 pt-6 sm:px-6 lg:px-8">
        <ImageGallery images={apartment.apartment_images} title={apartment.title || "Apartment"} />
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {apartment.reviews && apartment.reviews.length > 0 ? (
            apartment.reviews.map((review, idx) => (
...
// Skeleton for fallback
function ApartmentDetailSkeleton() {
  return (
    <div className="max-w-7xl mx-auto px-4 mt-8 space-y-8 animate-pulse">
